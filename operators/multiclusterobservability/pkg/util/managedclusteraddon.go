...
func needsConfigReferencesInitialization(ctx context.Context, c client.Client, mca *addonv1alpha1.ManagedClusterAddOn) (bool, error) {
	...
	for _, configRef := range mca.Status.ConfigReferences {
		if configRef.ConfigGroupResource.Group == AddonGroup &&
			configRef.ConfigGroupResource.Resource == AddonDeploymentConfigResource {
			...
			if configRef.DesiredConfig.ConfigReferent.Name != cmaDefaultConfig.Name ||
				configRef.DesiredConfig.ConfigReferent.Namespace != cmaDefaultConfig.Namespace {
		...
		if supportedConfig.ConfigGroupResource.Group == AddonGroup &&
			...
			for i := range mcaStatus.ConfigReferences {
				if mcaStatus.ConfigReferences[i].ConfigGroupResource.Group == AddonGroup &&
					...
					found = true
					// Update ConfigReferent fields if they changed
					...
					// Update DesiredConfig.ConfigReferent
